...
#| label: fig-maze
#| fig-align: center
#| fig-cap: Un labyrinthe
#| echo: false

n <- 2000
theta <- seq(1, 8 * pi, length.out = n)
...
data_maze <- rbind(data.frame(x = x1, y = y1, id = 1),
                   data.frame(x = x2, y = y2, id = 2))

ggplot(data_maze, aes(x, y, group = id)) +
...
: Java méthods visibility {#tbl-java-visibility .bordered}

---

Comme indiqué ici @tbl-java-visibility, c'est facile de citer un tableau.
...
Comme le spécifie cet article [@prospect_theory], la Prospect Theory c'est cool.
