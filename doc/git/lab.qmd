...
Durant ce TP, vous allez transférer du code entre votre dépôt local (service VSCode du datalab) et votre dépôt distant (sur GitHub). Pour valider ces transfert un processus d'authentification est naturellement requis.

### Génération et déclaration d'un Token

> Pour éviter d'avoir à vous authentifier (user+password) à chaque commande Git, vous allez utiliser un jeton.
...
::: {.callout-note title="Explication détaillée" collapse="true"}
`git config --global core.editor "code-server --wait"`
