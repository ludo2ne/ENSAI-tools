...
- [ ] Enable *AI Assistant*
  - Default model: *gemma4-26b-moe* or *qwen3-6-35b-moe*
  - Provider: *openai*
...
::: {.callout-caution}
Models hosted natively on SSPCloud have limited computing power. To check service availability and server load, please refer to <https://status.lab.sspcloud.fr/status/sspcloud>{target="_blank"}.

You can connect to third-party models (e.g., `codestral`) by modifying the Provider and API URL. Please note that using external services is subject to **token limitations**.
:::
...
::: {.callout-important}
Once more: Models hosted natively on SSPCloud have limited computing power.

If all students use the same model, it may cause the hosting server to crash due to resource exhaustion.
:::

To add other models, edit file `/home/onyxia/.continue/config.yaml`, adding new models:

```{.yml filename="config.yaml"}
models:
  - name: "qwen3-6-35b-moe"
    model: "qwen3-6-35b-moe"
    provider: "openai"
    apiBase: "https://llm.lab.sspcloud.fr/api/v1"
    apiKey: "??????"
...
For example, to use *codestral* (or another Mistral model):

- [ ] Create an account on [Mistral](https://console.mistral.ai)
- [ ] Create an [API key](https://console.mistral.ai/api-keys){target="_blank"}
- [ ] Keep an eye on your monthly token usage to avoid having your account blocked
...
- Capability: Explains code and provides snippets, but **cannot** modify your workspace.
- Best For: Explaining concepts, debugging snippets, or asking "how-to" questions.

Plan (The Architect):

- Role: Strategic planning.
- Capability: Deconstructs complex requests into a step-by-step roadmap **before** writing any code.
- Best For: Validating logic and strategy for large-scale changes or complex refactoring.

Agent (The Developer):

- Role: Autonomous execution.
- Capability: An autonomous loop that can **read files, edit code, and run terminal commands** to achieve a goal.
- Best For: End-to-end tasks (e.g., *"Implement X feature across the project"*) and auto-fixing bugs using terminal feedback.



What if you use the wrong mode?

Using Chat for complex tasks: You'll get long code snippets to copy-paste manually, but the AI won't touch your files or run tests. You do all the heavy lifting.

Using Plan for simple questions: The AI might over-analyze, giving you a "roadmap" for a task that only required a one-sentence answer.

Using Agent for trivial questions: You might waste tokens and time as the Agent scans your entire directory and attempts to "verify" things that didn't need it.
:::
